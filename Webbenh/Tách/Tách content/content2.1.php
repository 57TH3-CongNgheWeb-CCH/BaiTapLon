		<div id="content-column1">
		
			</a>
			<a href="webbenh.php?webbenh1=Beophi1&id=13" id="a">
				<img src="../images/benhbeophi.jpg" id="img">
				<p id="p">Béo phì</p>
			</a>
			<a href="webbenh.php?webbenh1=Soithan1&id=14" id="a">
				<img src="../images/benhsoithan.jpg" id="img">
				<p id="p">Sỏi thận</p>
			</a>
		</div>
		<div id="content-column2">
			<a href="webbenh.php?webbenh1=Huyetap1&id=15" id="a">
				<img src="../images/huyetap.jpg" id="img">
				<p id="p">Huyết áp</p>
			</a>
			<a href="webbenh.php?webbenh1=Timmach1&id=16" id="a">
				<img src="../images/Timmach.jpg" id="img">
				<p id="p">Tim mạch</p>
			</a>
			<a href="webbenh.php?webbenh1=Matngu1&id=17" id="a">
				<img src="../images/matngu.jpg" id="img">
				<p id="p">Mất ngủ</p>
			</a>
		</div>

		<div id="content-column3">
			<a href="webbenh.php?webbenh1=Tieuduong1&id=12" id="a">
				<img src="../images/tieuduong.jpg" id="img">
				<p id="p">Tiểu đường</p>
			</a>
			<a href="webbenh.php?webbenh1=Tri1&id=12" id="a">
				<img src="../images/tri.jpg" id="img">
				<p id="p">Bệnh trĩ</p>
			</a>
			<a href="webbenh.php?webbenh1=Mat1&id=12" id="a">
				<img src="../images/mat.jpg" id="img">
				<p id="p">Bệnh về mắt</p>
			</a>
		</div>